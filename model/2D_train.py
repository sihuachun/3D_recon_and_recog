...
class Config():
    ...
    def create_data_couple():
        ...
        data_couple = []
        # 正样本对数据集
        same_path = glob.glob('/home/aistudio/datasets/2D/same/*')
        for path in same_path:
            sub_path = glob.glob(path + '/*.png')
        ...
        # 负样本对数据集
        paths = glob.glob(Config.training_dir + '/train/*.png')
...
class Dataset():
    def my_transform(self, Objects):
        ...
        for Object in Objects:
            Object = cv2.resize(Object, shape[1:], interpolation=cv2.INTER_LINEAR)
