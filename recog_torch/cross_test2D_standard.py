...
class SiameseNetworkDataset(Dataset):
    ...
    def my_transform(self, Object):
        mid = Object.shape[0] // 2
        full = np.zeros(shape=shape)
        full[:, :Object.shape[1], :Object.shape[2]] = np.array([Object[mid]])
        return torch.from_numpy(full)

    def generate(self):

        img0 = tifffile.imread(self.f_path)
        img1 = tifffile.imread(self.s_path)
...
class SiameseNetwork(nn.Module):
    def __init__(self):
        ...
        self.cnn1 = nn.Sequential(
            ...
            nn.Conv2d(shape[0], 4, kernel_size=4, stride=2),
            nn.Dropout(0.3),
...
if __name__ == "__main__":
    ...
    check_point = torch.load('model_2D_standard.pkl', map_location=torch.device('cpu'))
    ...
    df.to_csv("cross2D_standard_{}.csv".format(test_name), index=False)
